...
<body class="flex flex-col h-screen justify-between bg-gray-50 text-gray-700">
    ...
    <!-- Responsive navbar-->
    <nav class="flex items-start justify-between flex-wrap bg-sky-800 p-6">
        <div class="flex items-center flex-shrink-0 text-white mr-6">
            ...
            <div>
                ...
                <!-- Leagues dropdown -->
                <div class="relative h-6 w-44 {{ $allLeagues->count() < 1 ? 'hidden' : '' }}">
                    <form id="leagueForm" method="post">
                        @csrf
                        @method('PATCH')
                        <input type="hidden" id="active" name="active" value="1">
                        <select id="league_id" name="league_id" onchange="updateAction(this.value)"
                            class="absolute border border-white bg-sky-800 text-xs inset-0 pl-2 pr-6 leading-none appearance-none w-full rounded">
                            @foreach ($allLeagues as $league)
                                <option value="{{ $league->id }}"
                                    {{ $league->id == $activeLeague->id ? 'selected' : '' }}>{{ $league->name }}
                                </option>
                            @endforeach
                        </select>
                    </form>
                    <div class="pointer-events-none absolute right-0 top-1 flex text-withe">
                    ...
                    <script>
                        function updateAction(value) {
                            ...
                            var url = "{{ route('leagues.activate', ':id') }}";
            ...
            <!-- Active league -->
            {{-- <span class="inline-block text-sm px-4 py-2 leading-none border rounded text-white border-white ml-6 {{ $activeLeague ? '' : 'invisible' }}">{{ $activeLeague->name ?? '' }}</span> --}}
        ...
        <!-- Menu -->
        <div id="mobile-menu" class="hidden w-full flex-grow lg:flex lg:items-center lg:w-auto mt-3">
